...
const child=spawn(process.env.CHROME_PATH||'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe',['--remote-debugging-port=9334',`--user-data-dir=${join(tmpdir(),`aegis-audit-${process.pid}`)}`,'--headless=new','--disable-gpu','--no-first-run','--window-size=1600,1100','http://localhost:3008/login'],{stdio:'ignore'});
const delay=ms=>new Promise(r=>setTimeout(r,ms));async function target(){for(let i=0;i<80;i++){try{const p=(await fetch('http://127.0.0.1:9334/json/list').then(r=>r.json())).find(x=>x.type==='page');if(p)return p}catch{}await delay(250)}throw new Error('Chrome did not start')}
