...
`apps/web/scripts/smoke-suite.mjs` starts a headless Chrome session against port 3008, authenticates, visits Command Center plus all 26 modules, verifies headings and deep links, completes the dormant-E5 assignment/remediation path, exercises 26 governed workflow CTAs plus five stateful suite outcomes, validates five populated PDF exports and report Excel/PDF downloads, changes a custom-report semantic source, adds a field, opens preview/schedule/security, saves the report, adds a dashboard widget, validates versioned configuration persistence, and captures screenshots. Supply the local password through `AEGIS_SMOKE_PASSWORD`; never embed it in the script.

## Honest implementation boundary

The local authentication boundary, UI, synthetic data model, tenant-scoped API, persistent finding/workflow/report runtime, audit protections, exports, deployment definitions, and security pipeline are executable. Finding evidence, report rows, connector states, and AI responses are deterministic synthetic demonstrations. Live Microsoft 365 data and mutations require customer-specific collectors, application registrations, certificates/managed identities, least-privilege consent, production repositories/workers, notification credentials, and the release gates in `docs/roadmap.md`.
